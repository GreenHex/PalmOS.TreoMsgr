...
Char *ToDoDBRecordGetFieldPointer( ToDoDBRecordPtr recordP,
 ...
 *
 *************************************************************/
Int16 DateTypeCmp(DateType d1, DateType d2)
{
	Int16 result;
...
//		ToDoDBRecordSetRepeatInfo( dbP, *index, gRepeatAfterCompleted,
...
void ToDoSetDBBackupBit(DmOpenRef dbP)
{
	...
	DmDatabaseInfo(cardNo, dbID, NULL, &attributes, NULL, NULL,
			&modDate, NULL, &modNum, NULL, NULL, NULL, NULL);
...
Err ToDoGetDatabase (DmOpenRef *dbPP, UInt16 mode)
{
  ...
  // Find the application's data file.  If it doesn't exist create it.
	dbP = DmOpenDatabaseByTypeCreator (toDoDBType, kPalmOneCreatorIDTasks, mode);
	if (!dbP)
	{
		error = DmCreateDatabase (0, toDoDBName, kPalmOneCreatorIDTasks, toDoDBType, false);
		if (error)
			return error;
